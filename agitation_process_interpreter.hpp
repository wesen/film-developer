...
typedef struct {
  ...
  const char *user_message; // Points to current wait message if
                            // waiting_for_user is true
} AgitationProcessInterpreterState;

// Initialize process interpreter
void agitation_process_interpreter_init(AgitationProcessInterpreterState *state,
                                        const AgitationProcessStatic *process,
...
void agitation_process_interpreter_confirm(
    AgitationProcessInterpreterState *state);